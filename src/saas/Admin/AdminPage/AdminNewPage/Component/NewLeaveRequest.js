...
const NewLeaveRequest = () => {
            ...
            options={[
              {
                label: "Pending",
                value: "Pending",
                disabled: statuses[record?.id] !== "Pending",
              },
  ...
  const handleLeaveStatus = (e) => {
    ...
    setFormData({ ...formData, leaveStatus: e.target.value });
  };

  useEffect(() => {
    const filterEmployeeData = () => {
      let filteredData;
      if (formData.leaveStatus === "All Data") {
        filteredData = leaveData?.filter((item) => item?.request_for !== "WFH");
      } else {
        filteredData = leaveData
          ?.filter((item) => item?.request_for !== "WFH")
          ?.filter((emp) => {
            console.log("ZXcxzxxxxx",emp)
            const matchesLeave =
              formData.leaveStatus === "All Data" ||
              emp?.status === formData.leaveStatus;

            return matchesLeave;
          });
      }

      setFilterLeaveData(filteredData);
    };

    filterEmployeeData();
  }, [formData.leaveStatus, leaveData, dispatch]);
  return (
