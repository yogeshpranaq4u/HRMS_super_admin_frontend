...
import { getSalary } from "../../../../redux/actions/employeeActions";
...
const NewSalaryPage = () => {
  ...
  const salaryCount =useSelector((state)=> state?.employeeData.salaryData);
  ...
  useEffect(() => {
    if(getEmployeeDetails){
      dispatch(getSalary(getEmployeeDetails[0]?.id));
    }
  }, [getEmployeeDetails]);
  ...
  useEffect(() => {
    if (getEmployeeDetails.length > 0) {
      getEmployeeSalaryDetails(currentEmpIndex);
      allSalary();
    }
  ...
  const handleRowClick = (employeeId) => {
    ...
    // console.log("handle row ss" ,index ,employeeId);
    dispatch(getSalary(employeeId));
        ...
        if (response?.data?.authenticated === false) {
          ...
          if (response?.data?.valid === false) {
            ...
            if (response?.data?.success === true) {
              ...
              setLoading(false);
              setSalaryDays(0);
              dispatch(getSalary(getEmployeeDetails[currentEmpIndex]?.id))
              getEmployeeSalaryDetails(currentEmpIndex);
              allSalary();
            } else {
  ...
  const allSalary = async () => {
    ...
    try {
      ...
      if(response.authenticated && response.valid){
        ...
        setEmployeeSalaryHistory(response?.data)
        return
      }
                ...
                <div className="flex items-center">
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <div className="w-12 h-6 bg-gray-300 rounded-full peer-checked:bg-blue-600 relative flex items-center">
                      {/* Toggle Thumb */}
                      <div
                        className={`absolute top-1/2 left-1 transform -translate-y-1/2 w-5 h-5 bg-white rounded-full transition ${weekOn ? "translate-x-6" : ""
                          }`}
                      ></div>
                    </div>
                  </label>
                    ...
                    <div className="p-4 rounded-lg bg-white shadow-sm">
                      <div className="flex items-start space-x-6 p-4">
                        {getEmployeeDetails[currentEmpIndex]?.image == null ? (
                          ...
                          <img
                            src={
                              ImagePath + getEmployeeDetails[currentEmpIndex]?.image
                            }
                            alt="User"
