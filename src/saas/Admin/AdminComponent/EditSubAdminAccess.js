...
const EditSubAdminAccess = ({ open, onClose, data }) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      const stringifiedPages = data?.pages; // The string data you received
      const parsedPages = JSON.parse(stringifiedPages); // Convert string to array

      setSelectedOptions(parsedPages); // Set the parsed array to state
    }
  }, [data]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ marginBottom: "20px" }}>Edit Employee To Access Page</h2>
        </div>
...
export default EditSubAdminAccess;
